...
def init_db():
    ...
    if c == 0:
        classrooms = [
            ("Smartboard Room 1", 25, "Interactive Whiteboard, Projector, Audio System"),
            ("Smartboard Room 2", 30, "Interactive Whiteboard, Projector, Audio System, Document Camera"),
            ("Conference Room A", 15, "Smart TV, Video Conferencing Setup"),
        ]
        db.executemany("INSERT INTO classrooms (name, capacity, equipment) VALUES (?,?,?)", classrooms)
        ...
        <form id="booking-form" method="post" action="{{ url_for('book') }}">
          <div class="form-group">
            ...
            <select name="classroom_id" id="classroom_id" required>
              {% for c in classrooms %}
                <option value="{{c['id']}}">{{c['name']}} (Capacity: {{c['capacity']}})</option>
              {% endfor %}
            </select>
          </div>
...
def admin():
    ...
    if request.method == "POST":
        ...
        if action == "reset_password":
            ...
            if name:
                try:
                    db.execute("INSERT INTO classrooms (name, capacity, equipment) VALUES (?,?,?)", 
                    ...
                    flash("Invalid capacity number.", "error")
            else:
    ...
    <div class="container">
      ...
      <form method="post" style="display:flex; gap:8px; align-items:end; margin-bottom: 15px;">
        <div>
        ...
        <div>
          ...
          <input name="capacity" type="number" value="30" min="1" max="200">
        </div>
        <div>
      ...
      <table class="table">
        <thead><tr><th>Room Name</th><th>Capacity</th><th>Equipment</th></tr></thead>
        <tbody>
          {% for c in classrooms %}
            <tr>
